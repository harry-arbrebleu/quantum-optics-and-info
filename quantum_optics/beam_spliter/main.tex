...
\begin{document}
  ...
  \subsection{Baker-Campbell-Hausdorffの公式2}
    ...
    \begin{proof}
      ...
      $\log (1 + x)$は$x = 0$のまわりで，
      \begin{align}
        \log(1 + x) &= \qty(\log(1 + x))\eval_{x = 0} + \frac{1}{1!}\qty(\dv{x}\log(1 + x))\eval_{x = 0}x + \frac{1}{2!}\qty(\dv[2]{x}\log(1 + x))\eval_{x = 0}x^2 + \frac{1}{3!}\qty(\dv[3]{x}\log(1 + x))\eval_{x = 0}x^3 + \cdots \\ 
